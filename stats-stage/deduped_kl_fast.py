...
def process_file_pair(probs1, file2_path):
    probs2 = cp.asarray(np.load(file2_path)[:, :50304])
...
for model_name_1 in model_names:
    for revision_1 in revisions:
        for i in [1,5,9]:
            ...
            output_file = f'../results/deduped/{model_name_1.replace("/", "-")}-{revision_1}-seed{i}-kl.npy'
            if os.path.exists(output_file):
                print(f"Skipping calculation for {output_file} as it already exists.")
                continue

            probs1 = cp.asarray(np.load(os.path.join(base_dir_1, files_1[0]))[:, :50304])
