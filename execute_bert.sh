...
python runner_bert.py "$WORKING_DIR" 0,1,2,3,4 || { echo "Failed to run runner_seeds.py"; exit 1; }
...
python bert_stats.py "$WORKING_DIR" || { echo "Failed to run seeds_stats.py"; exit 1; }
python bert_uni.py "$WORKING_DIR" || { echo "Failed to run seeds_stats.py"; exit 1; }