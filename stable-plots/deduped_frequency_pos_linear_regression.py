...
df = pd.read_csv(f'../working_dir/{sys.argv[1]}/output/deduped_frequency_dataframe.csv')
df = df[df['Model 1'] == df['Model 2']]
# Log the frequency column
...
def one_revision_LG(df):
    ...
    # Prepare X and y
    X = df_encoded.drop(columns=['KL', 'Revision 1', 'Revision 2', 'Model 2', 'Unnamed: 0'])
    y = df_encoded['KL']
...
df_transpose.to_csv(f'../working_dir/{sys.argv[1]}/output/deduped_frequency_pos_linear_regression_transpose.csv')
