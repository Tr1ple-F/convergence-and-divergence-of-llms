...
python tokenize_util "$WORKING_DIR" || { echo "Failed to run tokenize_util"; exit 1; }
...
python seeds_kl_torch.py "$WORKING_DIR" || { echo "Failed to run seeds_kl_torch.py"; exit 1; }
...
tar -czvf "$output_file" ./$WORKING_DIR/results/seeds || { echo "Failed to create tar.gz file"; exit 1; }
